...
Particles::Particles(int _maxParticles, string filePath_meme, string filePath_first){
    ...
    outside = ofSpherePrimitive(10, wire).getMesh();
...
void Particles::draw(){
    ...
    for (int i = 0; i < mesh.getVertices().size(); i++) {
        ...
        float posture_loc = ofMap(morph_posture, min_posture, max_posture, 0, 10);
        ofVec3f loc = mesh.getVertices()[i] / posture_loc;

        float noise = ofMap(ofNoise(loc.x, loc.y, loc.z, ofGetElapsedTimef()), 0, 1, 8,24);
        ...
        mesh.setColor(i, ofFloatColor(R,G,B,0.5));
        outside.setColor(i, ofFloatColor(1.0, 1.0, 1.0, 0.5));
        mesh_line.setColor(i, ofFloatColor(R,G,B,0.8));
    }
